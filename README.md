...
- **AI Match Engine**: Powered by Google's **Gemini AI API** (`@google/genai` using models like `gemini-3.1-flash-lite` and `gemini-2.5-flash-lite`) to generate quantitative fit scores (0-100%), detailed match reasons, missing skills gaps, and tailored cover letter action items. Automatically falls back to a deterministic **ATS Heuristic Engine** if an API key is missing or invalid.
